...
export default function HandoverCenterPage() {
  ...
  const resetEditedPreview = (type = previewType) => setPreviewOverrides((current) => ({ ...current, [type]: null }));
  ...
  return <HandoffDetail type={mode} step={step} conditions={activeConditions} candidates={activeCandidates} selectedIds={activeIds} previewData={preview} onBack={() => openMode("home")} onStep={setStep} onCondition={changeCondition} onInclude={toggleInclude} onToggle={toggleCandidate} onEdit={(nextPreview) => setPreviewOverrides((current) => ({ ...current, [previewType]: nextPreview }))} onRegenerate={() => { resetEditedPreview(); toast("최신 업무 자료로 미리보기를 다시 생성했습니다."); }} onShare={share} onSave={save} />;
}
