...
export default function HandoffPreview({ previewData, type, savedDraftId, onPreviewChange, onRegenerate, onShare, onSave, onOpenArchive, archived = false }) {
  ...
  const handleAiQuestion = (question) => {
    if (typeof window.openAssistantWithQuestion === "function") {
      window.openAssistantWithQuestion(question);
    }
  };
  ...
  return <article className={`hc-preview${isEditingPreview ? " editing" : ""}`}>
    ...
    <div className="hc-preview-body">{displayed.sections.map(([label, values], index) => {
      const isAiQuestions = label === "AI Assistant에 물어볼 질문";
      return (
        <section className={`hc-preview-section${isAiQuestions ? " hc-ai-questions-section" : ""}`} key={`${label}-${index}`}>
          <h3>{label}</h3>
          {isEditingPreview
            ? <textarea value={values.join("\n")} onChange={(event) => updateSection(index, event.target.value)} aria-label={`${label} 내용 편집`} />
            : isAiQuestions
              ? <ul className="hc-ai-question-list">{values.map((value, itemIndex) => (
                  <li key={`${label}-${itemIndex}`}>
                    <button type="button" className="hc-ai-question-btn" onClick={() => handleAiQuestion(value)}>
                      <span className="hc-ai-question-text">{value}</span>
                      <span className="hc-ai-question-cta">AI에게 질문하기 →</span>
                    </button>
                  </li>
                ))}</ul>
              : <ul>{values.map((value, itemIndex) => <li key={`${label}-${itemIndex}`}>{value}</li>)}</ul>
          }
        </section>
      );
    })}</div>
