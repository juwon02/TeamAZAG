...
export default function HandoffPreview({ previewData, type, onPreviewChange, onRegenerate, onShare, onSave, archived = false }) {
  ...
  return <article className={`hc-preview${isEditingPreview ? " editing" : ""}`}><header className="hc-preview-head"><div><span>{isEditingPreview ? "DOCUMENT EDIT MODE" : archived ? "ARCHIVED DOCUMENT" : "AI GENERATED PREVIEW"}</span><h2>{displayed.title}</h2></div><span className="hc-status">{isEditingPreview ? "편집 중" : "초안"}</span></header><div className="hc-preview-body">{displayed.sections.map(([label, values], index) => <section className="hc-preview-section" key={`${label}-${index}`}><h3>{label}</h3>{isEditingPreview ? <textarea value={values.join("\n")} onChange={(event) => updateSection(index, event.target.value)} aria-label={`${label} 내용 편집`} /> : <ul>{values.map((value, itemIndex) => <li key={`${label}-${itemIndex}`}>{value}</li>)}</ul>}</section>)}</div><footer className="hc-preview-actions">{isEditingPreview ? <><button type="button" onClick={cancelEditing}><i className="ti ti-x" />취소</button><button type="button" className="primary" onClick={saveEditing}><i className="ti ti-device-floppy" />저장</button></> : <><button type="button" onClick={startEditing}><i className="ti ti-edit" />수정하기</button>{!archived && <><button type="button" onClick={() => onRegenerate(type)}><i className="ti ti-refresh" />다시 생성</button><button type="button" onClick={onShare}><i className="ti ti-share" />공유하기</button><button type="button" className="primary" onClick={onSave}><i className="ti ti-device-floppy" />초안 저장</button></>}</>}</footer></article>;
}
