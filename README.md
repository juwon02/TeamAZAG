...
TeamAZAG는 프로젝트 문서, 할 일, 이슈, AI 채팅, 리포트, 인수인계 정보를 프로젝트 단위로 관리하기 위한 DB 초안.

## 현재 DB 방향

- 관계형 데이터베이스는 PostgreSQL을 사용.
- 문서 조각 임베딩 검색은 FAISS를 사용.
- PostgreSQL에는 임베딩 벡터 원본을 저장하지 않음.
- PostgreSQL에는 `chunk_embeddings.vector_store`, `chunk_embeddings.vector_id` 같은 FAISS 참조 정보만 저장.
- AI가 추출한 할 일과 사람이 직접 만든 할 일은 모두 `todos` 테이블에 저장.
- Todo 화면에서는 `source_type`과 `approval_status` 값으로 AI 할 일과 공식 할 일을 구분.
...
먼저 PostgreSQL에서 개발용 데이터베이스를 만든다.

```bash
createdb teamazag
```

그 다음 아래 순서로 SQL 파일을 실행.
...
AI가 뽑아낸 할 일 후보와 사람이 직접 만든 공식 할 일은 같은 `todos` 테이블을 사용.
